...
Heuristic model: 
    ...
    over-here, considering only one slice like URLLC and at possion rate the services are coming and cauing the insufficient functionality.
...
class Network:
    ...
    def simulate_network_operation(self):
        for slice_name, slice_data in self.slice_definitions.items():
            ...
            #     serving_base_station.connect_user(user, slice_name)

            #     # Print details about the serving base station
            #     #print(f"Serving Base Station ID: {serving_base_station.id} at Location: {serving_base_station.location}")

            #     snr = serving_base_station.calculate_snr(user)
            #     cqi = serving_base_station.calculate_cqi(snr)
            #     #print(f"Slice {slice_name}: User {user.id} ({user.type}) --> {cqi}")
...
class BaseStation:
    ...
    def calculate_snr(self, user, use_hata=False):
        ...
        #(f"received_power: {received_power}")
